...
##wrong cause for numeric addiction
# dat <- dat %>%
#   rowwise() %>%
#   mutate(
#     other_rating = rnorm(
...
#              interaction_effect * (self_rating * (addiction_n)), # interaction
...
##new try
# Define base interaction slopes per addiction
interaction_effects <- runif(n_addictions, -0.05, 0.05)  # each addiction gets its own "true" interaction strength
  ...
  mutate(
    other_rating = rnorm(
      ...
      mean = 2.28 +
             ...
             interaction_effects[as.numeric(addiction)] * self_rating,  # lookup correct slope
      sd = sqrt((1 - r_so^2) * sd_self^2) * resid_inflation
    )
  ) %>%
  ungroup()
